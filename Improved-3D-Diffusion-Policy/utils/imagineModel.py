...
class G3FlowVirtualSpace(gym.Env):
    ...
    def load_camera(self, center_point, obj_quat, camera_w = 320,camera_h = 240):
        ...
        cam_height = 0.3
        delta = 0.3
        obj_matrix = t3d.quaternions.quat2mat(obj_quat) @ np.eye(3)
    ...
    def load_actor(self, actor_file_path, actor_data_path = None, actor_world_pose = sapien.Pose(p=[0, 0, 0.]) ):
...
def create_actor(
    ...
    #     model_data = None
    model_data = None
    # actor = loader.load(file_name)
    actor = loader.load_multiple(file_name)
    # mesh = loader.build()
    # mesh.set_pose(pose)
